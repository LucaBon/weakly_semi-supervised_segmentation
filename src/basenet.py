...
class Normalize:
    ...
    def undo(self, img_arr):
        proc_img = img_arr.copy()

        proc_img[..., 0] = (self.std[0] * img_arr[..., 0] +
                            self.mean[0]) * 255.
        proc_img[..., 1] = (self.std[1] * img_arr[..., 1] +
                            self.mean[1]) * 255.
        proc_img[..., 2] = (self.std[2] * img_arr[..., 2] +
                            self.mean[2]) * 255.
    ...
    def __call__(self, img):
        ...
        proc_img = np.empty_like(img_arr, np.float32)

        proc_img[..., 0] = (img_arr[..., 0] / 255. - self.mean[0]) / self.std[0]
        proc_img[..., 1] = (img_arr[..., 1] / 255. - self.mean[1]) / self.std[1]
        proc_img[..., 2] = (img_arr[..., 2] / 255. - self.mean[2]) / self.std[2]
...
class BaseNet(nn.Module):
    ...
    def parameter_groups(self, base_lr, wd):
        ...
        groups = ({"params": [], "weight_decay": wd, "lr": w_old * base_lr},
                  ...
                  # bias finetuning
                  {"params": [], "weight_decay": wd, "lr": w_new * base_lr},
                  # weight finetuning
                  {"params": [], "weight_decay": 0.0,
                   "lr": b_new * base_lr})  # bias learning
        ...
        for i, g in enumerate(groups):
            print("Group {}: #{}, LR={:4.3e}".format(i, len(g["params"]),
                                                     g["lr"]))
